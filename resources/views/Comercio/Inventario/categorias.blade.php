                        ...
                        <table id="tabla_listado" class="table table-striped table-bordered dataTable no-footer table-hover" style="width: 100%;" role="grid" aria-describedby="datatable_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" style="width: 30%;">NOMBRE CATEGORIAS</th>
...
@component('Comercio.Componentes.modal_crear_clasificadores')
    ...
    @endslot
    @slot('descripcion')
        Describa el nombre de la Categoría a crear
    @endslot
    @slot('columna')
    @endslot
    @slot('abreviatura')
...
@component('Comercio.Componentes.modal_eliminar_clasificadores')
    ...
    @endslot
@endcomponent
{{-- #################### FINAL MODAL ELIMINAR CATEGORÍA ######################## --}}
...
<script>
   $(document).ready(function () {
        $('#tabla_listado').DataTable({
            ...
            "serverSide":true,
            "ajax":{
                url:"{{route('categorias_articulos')}}",
            },
            "columns":[
                {data:'DT_RowIndex'},
                {data:'nombre'},
    ...
    //###############  FUNCIÓN MODAL PARA CREAR CATEGORÍAS   #############
    function modalCrear(){
    ...
    function envioCrear(){
        ...
        formData.append('nombre_categoria',$('#nombreClasificador').val())

        // ENVIO AJAX
        var routeCrear = "{{asset('comercio/inventario/clasificadores/categorias/crear')}}";
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Crear Categorías");
            } else {
                $('#modalCrear').modal('hide');
                $('#nombreClasificador').val('');
                toastr["success"](res.mensaje,"Crear Categorías");
    ...
    //############### FIN FUNCIÓN MODAL PARA CREAR CATEGORÍAS  #############


    //###############  FUNCIÓN MODAL PARA EDITAR CATEGORÍAS   #############
    function modalEditar(id_categoria,nombre){
        $("#nombreNuevoClasificador").val(nombre);
        $("#id_clasificador").val(id_categoria);
    ...
    function envioModificar(){
        ...
        var formData = new FormData();
        formData.append('nombre_categoria',$('#nombreNuevoClasificador').val())
        formData.append('id_categoria',$('#id_clasificador').val())

        // ENVIO AJAX
        var routeModificar = "{{asset('comercio/inventario/clasificadores/categorias/modificar')}}";
        var token = $("#_token").val();
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Modificar Categorías");
            } else {
                $('#modalEditar').modal('hide');
                $('#nombreNuevoClasificador').val('');
                toastr["success"](res.mensaje,"Modificar Categorías");
                $("#tabla_listado").DataTable().ajax.reload();
            }
        })
        .fail(function(msg){
    ...
    //############### FIN FUNCIÓN MODAL PARA EDITAR CATEGORÍAS  #############

    //############### FUNCIÓN MODAL PARA ELIMINAR CATEGORÍAS  #############
    function modalEliminar(id_categoria){
        $("#id_clasificador").val(id_categoria);
    ...
    function envioEliminar(){
        ...
        // ENVIO AJAX
        var routeModificar = "{{asset('comercio/inventario/clasificadores/categorias/eliminar')}}";
        var token = $("#_token").val();
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Eliminar Categorías");
            } else {
                $('#modalEliminar').modal('hide');
                toastr["success"](res.mensaje,"Eliminar Categorías");
                $("#tabla_listado").DataTable().ajax.reload();
            }
        })
        .fail(function(msg){
    ...
    //############### FIN FUNCIÓN MODAL PARA ELIMINAR CATEGORÍAS  #############
</script>
@endsection
