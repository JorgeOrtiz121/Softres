...
<div class="col-md-12 col-sm-12 ">
    <div class="x_panel">
        ...
        <div class="text-left">
            <a onclick="modalCrear()"  title="Crear nuevo registro" class="btn .btn-lg btn-primary" style="color:white; cursor: pointer;">CREAR NUEVO</a>
            <a href="{{asset('comercio/inventario/clasificadores')}}" title="Volver a Clasificadores" class="btn .btn-lg btn-danger" >REGRESAR</a>
        </div>
        <div class="x_title">
            <h2> Listado de los Tipos </h2>
                        ...
                        <table id="tabla_listado" class="table table-striped table-bordered dataTable no-footer table-hover" style="width: 100%;" role="grid" aria-describedby="datatable_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" style="width: 30%;">NOMBRE TIPOS</th>
...
@component('Comercio.Componentes.modal_crear_clasificadores')
    ...
    @endslot
    @slot('descripcion')
        Describa el nombre del Tipo a crear
    @endslot
    @slot('columna')
    @endslot
    @slot('abreviatura')
...
@component('Comercio.Componentes.modal_eliminar_clasificadores')
    ...
    @endslot
    @slot('descripcion')
        ¿Realmente desea eliminar el Tipo?
    @endslot
@endcomponent
...
<script>
   $(document).ready(function () {
    ...
    //###############  FUNCIÓN MODAL PARA CREAR TIPOS   #############
    function modalCrear(){
    ...
    function envioCrear(){
        ...
        formData.append('nombre_tipo',$('#nombreClasificador').val())

        // ENVIO AJAX
        var routeCrear = "{{asset('comercio/inventario/clasificadores/tipos/crear')}}";
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Crear Tipos");
            } else {
                $('#modalCrear').modal('hide');
                $('#nombreClasificador').val('');
                toastr["success"](res.mensaje,"Crear Tipos");
    ...
    //############### FIN FUNCIÓN MODAL PARA CREAR TIPOS  #############


    //###############  FUNCIÓN MODAL PARA EDITAR TIPOS   #############
    function modalEditar(id_tipo,nombre){
        $("#nombreNuevoClasificador").val(nombre);
        $("#id_clasificador").val(id_tipo);
    ...
    function envioModificar(){
        ...
        var formData = new FormData();
        formData.append('nombre_tipo',$('#nombreNuevoClasificador').val())
        formData.append('id_tipo',$('#id_clasificador').val())

        // ENVIO AJAX
        var routeModificar = "{{asset('comercio/inventario/clasificadores/tipos/modificar')}}";
        var token = $("#_token").val();
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Modificar Tipos");
            } else {
                $('#modalEditar').modal('hide');
                $('#nombreNuevoClasificador').val('');
                toastr["success"](res.mensaje,"Modificar Tipos");
                $("#tabla_listado").DataTable().ajax.reload();
            }
        })
        .fail(function(msg){
    ...
    //############### FIN FUNCIÓN MODAL PARA EDITAR TIPOS  #############

    //############### FUNCIÓN MODAL PARA ELIMINAR TIPOS  #############
    function modalEliminar(id_tipo){
        $("#id_clasificador").val(id_tipo);
    ...
    function envioEliminar(){
        ...
        // ENVIO AJAX
        var routeEliminar = "{{asset('comercio/inventario/clasificadores/tipos/eliminar')}}";
        var token = $("#_token").val();
        ...
        .done(function(res){
            if (res.respuesta == 0) {
                toastr["error"](res.mensaje,"Eliminar Tipos");
            } else {
                $('#modalEliminar').modal('hide');
                toastr["success"](res.mensaje,"Eliminar Tipos");
                $("#tabla_listado").DataTable().ajax.reload();
            }
        })
        .fail(function(msg){
    ...
    //############### FIN FUNCIÓN MODAL PARA ELIMINAR TIPOS  #############
</script>
@endsection
